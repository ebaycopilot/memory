...
- The startup launcher now also runs `startup\Refresh-CopilotTokenAndLaunchVscode.ps1` first: it clears `GITHUB_COPILOT_TOKEN` and `SOCIAL_COMMENTER_API_KEY`, opens VS Code, waits for `oc-vscode-copilot-bridge-status.json` to report `ok=true` and `stage=done`, and verifies the refreshed token was written back before continuing.
